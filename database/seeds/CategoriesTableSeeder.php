...
class CategoriesTableSeeder extends Seeder
{
    ...
    public function run()
    {
        $cereals = App\Category::create(['alias'=>'cereals', 'name'=>'зерновые', 'translit'=>'zernovyye']);
        $cereals->products()->createMany([
        ...
        $oilseeds = App\Category::create(['alias'=>'oilseeds', 'name'=>'масличные', 'translit'=>'maslichnyye']);
        $oilseeds->products()->createMany([
            ["alias"=>"sunflower","name"=>"подсолнечник","translit"=>"podsolnechnik","available"=>"yes"],
            ["alias"=>"rape","name"=>"рапс","translit"=>"raps","available"=>"yes"],
        ]);
        
        $beans = App\Category::create(['alias'=>'beans', 'name'=>'бобовые', 'translit'=>'bobovyye']);
        $beans->products()->createMany([
